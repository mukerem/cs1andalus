...
if TkVersion < 8.0 :
	logging.info ("\n" * 3)
	logging.info ("*"*75)
	logging.info ("Running Tk version:", TkVersion)
	logging.info ("You must be using Tk version 8.0 or greater to use EasyGui.")
...
multenterbox can be checked for validity before they are accepted::
    ...
    logging.info "Reply was:", fieldValues
    ----------------------------------------------------------------------
...
def __choiceboxGetChoice(event):
	...
	# logging.info "Debugging> mouse-event=", event, " event.type=", event.type
	# logging.info "Debugging> choice =", choice_index, __choiceboxResults
	root.quit()
...
def KeyboardListener(event):
	...
	key = event.keysym
	if len(key) <= 1:
		if key in string.logging.infoable:
...
*5)+"\n\n")*10
	...
	while 1: # do forever
		...
		if   reply[0] == "msgbox":
			...
			logging.info ("Reply was:", repr(reply))
			...
			logging.info ("You chose directory...:", d)

		elif reply[0] == "fileopenbox":
			f = fileopenbox()
			logging.info ("You chose to open file:", f)

		elif reply[0] == "filesavebox":
			f = filesavebox()
			logging.info ("You chose to save file:", f)
			...
			logging.info ("Reply was:", str(reply))
