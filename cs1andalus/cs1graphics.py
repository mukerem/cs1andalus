...
try:
    ...
    try:
        import Image as _Image # python 2
...
class _Hierarchy:
    ...
    def computeUpwardChains(self, drawable, counts = None):
        ...
        if _debug >= 2:
            logging.info('ComputeUpwardChains('+str(drawable)+','+str(counts)+') returning:')
            for c in results:
                logging.info('    '+str(tuple(c)))
    ...
    def computeDownwardChains(self, drawTuple):
        ...
        if _debug >= 2:
            logging.info('ComputeDownwardChains('+str(drawTuple)+') returning:')
            for c in results:
                logging.info('    '+str(tuple(c)))
        return results
...
class _RenderedHierarchy:
    ...
    def changeDepth(self, chain, newDepth):
        ...
        if _debug >= 1.5:  logging.info('change depth of ' + str(chain) + ' from ' + str(oldDepth) + ' to ' + str(newDepth))
           ...
           (nextSib is not None and newDepth > nextSib.key()):
            ...
            if _debug >= 2.5:
                for (k,v) in iter(parent._sortedChildren):
                    logging.info( '  child: ' + str(k) + ' ' + str(v))
...
class _UpdateManager:
    ...
    class _node:
        ...
        def _resolveMirror(self, privateMap):
            ...
            # public branch.
            if _debug >= 2:
                logging.info("Within _resolveMirror on node " + str(self))
            
            for (chain, child) in list(privateMap):
                if _debug >= 3:
                    logging.info("Resolving child " + str(child) + " with status " + child._status + " and special " + child._special)
        ...
        def _updateRecurse(self, chain, style, properties={}, parentMap=None):
            ...
            if _debug >= 3:
                logging.info('_UpdateManager._node._updateRecurse called with\n    ' + '\n    '.join([str(x) for x in (self,chain,style,properties)]))
                logging.info('  Node is currently ' + ('frozen' if self.isFrozen() else 'unfrozen'))
            ...
            if self._chain == chain:
                ...
                if _debug >= 2.5:
                    logging.info("created new _UpdateManager._node: " + str(child) + " for chain " + str(chain))
        ...
        def _flushRecurse(self, parentMap=None):
          if _debug >= 3:
              logging.info('_flushRecurse called on node ' + str(self))
              logging.info('isFrozen currently' + str(self.isFrozen()))
    ...
    def flush(self):
        ...
        """
        if _debug >= 1:
            logging.info('_UpdateManager.flush() called')
...
class _GraphicsManager:
    ...
    def addCommandToQueue(self, command):
        ...
        if self._state != 'Failed':
          if _debug >= 1:
              logging.info('addCommandToQueue: ' + str(command))
    ...
    def processCommands(self):
        ...
        try:
            ...
            raise
                ...
                logging.info('Unknown graphics error has occured.  Graphics manager is shutting down.')
                logging.info('Program must be restarted to use graphics.')
                logging.info('If problem is repeatable, please report to bugs@cs1graphics.org.')
                self._state = 'Failed'
                self._closeAll()
                if _debug > 0:    # exit upon first error
                    _traceback.logging.info_exc(file=_sys.stdout)
                    _sys.exit()
    ...
    def processCommand(self, command):
        if _debug >= 1:
            logging.info('')
            logging.info('Manager executing: ' + str(command))
        ...
        # New objects
            ...
            if _debug >= 1:
                logging.info('_middleHierarchy.addLink: ' + str(containerTuple) + ' ' + str(drawableTuple))
            ...
            for d,count in downwardChains:
                ...
                for u in self._middleHierarchy.computeUpwardChains(containerTuple, count):
                    ...
                    if _debug >= 1.5:
                        ...
                        logging.info("Effective depth " + str(properties['depth']))
            ...
            if _debug >= 1:
                logging.info('_middleHierarchy.removeLink: ' + str(parent) + ' ' + str(child))
            ...
            if command[1] in self._middleHierarchy:
                for chain in self._middleHierarchy.computeUpwardChains(command[1]):
                    if 'depth' in command[2]:
                        ...
                        if _debug >= 1:
                            logging.info('Updating Effective Depth: %s' % str(command[2]['depth']))
    ...
    def _pushUpdates(self):
        ...
        if _debug >= 1:
            logging.info("_pushUpdates called")
        for (chain, status, properties) in self._updateManager.flush():
            if _debug >= 1:
                logging.info('_pushUpdates: ' + str(status)+' '+str(chain)+' '+str(properties))
                if self._renderedHierarchy.hasChain(chain):
                    logging.info('    Rendered Depth is ' + str(self._renderedHierarchy.getNode(chain)._depth))
            ...
            if status == 'add':
                ...
                if not isinstance(current._renderedDrawable, _RenderedCanvas) and current._renderedDrawable is not None:
                    ...
                    if node is not None and node._chain[0] == chain[0]:  # TODO: correct treatment of forest???
                        if _debug >= 1: logging.info('Putting '+str(current._renderedDrawable)+' above '+str(node._renderedDrawable))
                        current._renderedDrawable.putAbove(node._renderedDrawable)
                    else:
                        if _debug >= 1: logging.info('Putting '+str(current._renderedDrawable)+' at bottom')
                ...
                if 'depth' in properties:
                    ...
                    if first is not None:   # something changed
                        if _debug >= 1.5: logging.info('first, last = '+str( (first._renderedDrawable,last._renderedDrawable) ))
                        ...
                        while current != first._prev:
                            if current._renderedDrawable is not None:
                                if below is not None:
                                    if _debug >= 1.5:
                                        logging.info('Putting '+str(current._renderedDrawable)+' above '+str(below._renderedDrawable))
                                    current._renderedDrawable.putAbove(below._renderedDrawable)
                                else:
                                    if _debug >= 1.5:
                                        logging.info('Putting ' + str(current._renderedDrawable) +  ' at bottom')
                                    current._renderedDrawable.putAbove(None)
                                below = current
...
class _GraphicsContainer(object):
    ...
    def add(self, drawable):
        ...
        if self in _graphicsManager._frontHierarchy:
            if _debug >= 2: logging.info('adding drawable to "rendered" graphics container')
    ...
    def remove(self, drawable):
        ...
        if drawable in _graphicsManager._frontHierarchy:
            ...
            if _debug >= 1:
                logging.info('_frontHierarchy.removeLink: ' + str( (self,cls) ) + ' ' + str(childTuple))
...
def _wrapUtility(cls):
    if _debug >= 2: logging.info('_wrapUtility being called on class ' + str(cls))
    classDict = cls.__dict__
    if '_internalDraw' not in classDict:   # not alreadly wrapped
        if '_draw' in classDict:
            if _debug >= 2: logging.info('_wrapUtility: wrap was required')
            ...
            def drawClosure(self):
                ...
                if _debug >= 2: logging.info(str(cls) + ' draw wrapper called on ' + str(self))
                ...
                if not known:
                    if _debug >= 2: logging.info('about to call original _draw() for ' + str(self))
                    _graphicsManager._drawParent = (self,cls)
                    internalDraw(self)           # the original wrapped function, taken from closure
                    _graphicsManager._drawParent = parent
            
                if _debug >= 2: logging.info('draw wrapper call ending for ' + str(self))
            # end of closure
            #---------------------------------------------------------------------------
...
class _RenderedCanvas(object):
    def __init__(self, chain, properties):
        if _debug >= 1: logging.info('Creating _RenderedCanvas')
...
class _RenderedDrawable(object):
    ...
    def update(self, properties):
        if _debug >= 1: logging.info('Updating _RenderedDrawable')
        pass
...
def _exitMainThread():
    ...
    if len(_graphicsManager._openCanvases) > 0:
        logging.info('Close canvas windows to end program.')
